...
class AnotherPuzzle:
    ...
    def __init__(self, groups, clueset):
        self.items_per = len(groups[0])
        ...
        self.X = exprvars('x', (0, self.items_per), (0, self.items_per), (0, self.items_per), (0, self.items_per))
    ...
    def id_to_var(self, id):
        ...
        for i in range(self.num_groups):
            var = var + f'_{self.groups[i][id[i]]}'
    ...
    def are_same(self, value1, value2):
        ...
        #                 for idx in range(0, self.items_per) ])
        x_index = x_index[:group1_] + val_1 + x_index[group_1 + 1:]
        x_index = x_index[:group2_] + val_2 + x_index[group_2 + 1:]
        f_aresame = OneHot(*[ self.X[group, idx, root]
                                for root in range(0, self.items_per)
                                for
    ...
    def dnf_formula(self):
        ...
        print(esp_form)
        pprint(expr2truthtable(esp_form))
...
ex_puzzle = AnotherPuzzle(groups, [])
ex_puzzle.run()
